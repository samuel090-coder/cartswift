...
export class VoiceAssistant {
  ...
  private onFunctionCallCallback: ((functionName: string, args: any) => void) | null = null;
  ...
  constructor(
    ...
    this.onFunctionCallCallback = onFunctionCall || null;
  }

  async connect() {
    this.audioContext = new AudioContext({ sampleRate: 24000 });
    
    const wsUrl = `wss://${this.projectRef}.supabase.co/functions/v1/voice-assistant`;
    ...
    this.ws.onmessage = async (event) => {
      ...
      if (data.type === 'response.audio_transcript.done') {
        if (this.currentTranscript && this.onMessageCallback) {
      ...
      if (data.type === 'conversation.item.input_audio_transcription.completed') {
        if (data.transcript && this.onMessageCallback) {
          this.onMessageCallback({
            role: 'user',
  ...
  async startRecording() {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      throw new Error('Not connected to voice assistant');
    }
